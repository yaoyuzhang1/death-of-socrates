...
273个阅读页都有固定对应的插画，相邻272次翻页均更换画面。场景包括港口挽留、入室聚谈、克法洛斯离席、玻勒马霍斯接谈、色拉叙马霍斯的争辩、兄弟二人的发言，以及原文中的工匠、市场、牧人、音乐训练等例子。长讨论会在适用的原文范围内复用部分不同镜头；并非273幅各不重复的图片。

交谈镜头表现正在参与讨论的人物，例证图标为“谈话中的图景”。例证图只在相关正文段落范围内使用，提问页和复习摘录只用交谈镜头。分配按全文的实际顺序判断原段落范围，避免用问题ID的数字顺序代替阅读顺序。选项、解释及原文没有烧录到图片中，也不随玩家所选答案改变。

构建脚本`scripts/build-illustrations.mts`检查真实WebP文件并生成`reader-public/illustrations/manifest.json`，记录全部图片的文件名、说明、校验值和逐页映射。画面在手机上保留16:9比例，可点击放大；同侧下一正文页的图像提前加载，题后图像不会在题前预取。

完整的逐幅提示词、参考图和原文范围保存在以下清单；这些清单是内置工具的制作记录：

- [前四章17幅及提示词](art-prompts-early.md) · `content/art/early-scenes.json`
- [后四章18幅及提示词](art-prompts-late.md) · `content/art/late-scenes.json`
- [16幅交谈镜头及提示词](art-prompts-dialogue.md) · `content/art/dialogue-scenes.json`
