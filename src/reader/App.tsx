...
type Screen = 'home' | 'read' | 'chapter-end';
...
export default function App() {
  ...
  const choose = (choiceId: string | null) => {
    if (!question) return;
    if (practicing) setPractice({ questionId: question.id, choiceId, hinted: practice.hinted });
  ...
  return <div className="app" style={{ '--reading-font-size': `${save.settings.fontSize}px` } as CSSProperties}>
    ...
    <header className="site-header"><div className="header-inner">
      ...
      <nav aria-label="阅读工具"><button onClick={() => setPanel('contents')}><List size={18} /><span>目录</span></button><button onClick={() => setPanel('settings')}><Settings2 size={18} /><span>阅读设置</span></button></nav>
      ...
      <section className="home-contents" aria-labelledby="contents-title"><div className="section-heading"><h2 id="contents-title">从日常判断，读到城邦与灵魂</h2><p>按讨论的主题分章，可以随时暂停、接着读。</p></div>
        <div className="chapter-grid">{corpus.chapters.map((chapter, i) => {
          ...
          return <button className="chapter-row" key={chapter.id} onClick={() => accessible ? goTo(index) : setPanel('contents')}>
            <span className="chapter-number">{String(i + 1).padStart(2, '0')}</span><span className="chapter-label"><strong>{chapter.title}</strong><small>{chapter.range}</small></span><span className="chapter-mark">{done ? <Check size={18} /> : accessible ? <ArrowRight size={17} /> : <span>待阅读</span>}</span>
          </button>;
        })}</div>
      </section>
