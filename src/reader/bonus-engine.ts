...
export function navigateBonus(save: Save, units: readonly ReadingUnit[], index: number, access: BonusAccess = 'earned'): Save {
  if (!accessible(save, units, access) || !Number.isInteger(index) || index < 0 || index > bonusFrontier(save)) return save;
  if (index === save.bonus.cursor) return save;
  return { ...save, bonus: { ...save.bonus, cursor: index }, updatedAt: new Date().toISOString() };
}

export function finishBonus(save: Save, units: readonly ReadingUnit[], access: BonusAccess = 'earned'): Save {
  if (save.bonus.completed || !accessible(save, units, access) ||
