...
export default function LearningCheck({ check, record, onAnswer, audio }: {
  ...
  const play = (id: string, cue: boolean) => playRecordedFeedback(`study:${check.id}--${id}`, `${import.meta.env.BASE_URL}audio/study/${encodeURIComponent(check.id)}--${encodeURIComponent(id)}.mp3`, id === check.correctId, { ...audio, sound: cue && audio.sound });
  const confirm = () => {
  ...
  return <section className="learning-check" data-testid="learning-check" data-check-id={check.id} aria-labelledby={`prompt-${check.id}`}>
    ...
    <div className="study-confirm"><span>{selected ? '确认前还可以修改' : '选一个理解，再确认'}</span><button className="primary" disabled={!selected} onClick={confirm} data-testid="study-confirm">确认判断</button></div></> : <div className={`study-result ${correct ? 'study-correct' : 'study-wrong'}${attempt ? ' study-animate' : ''}`} key={run} ref={result} tabIndex={-1} role="status" data-testid="study-result">
      ...
      <p>{explanation}</p><div className="study-feedback-tools"><button className="text-button" disabled={!audio.voice} onClick={() => isPlaying ? stopFeedback() : play(choiceId, false)}>{isPlaying ? <Square size={15} /> : <Volume2 size={16} />}{isPlaying ? '停止朗读' : '重听解释'}</button><span>{!audio.voice ? '配音已关闭' : voice.key === audioKey && (voice.status === 'error' || voice.status === 'blocked') ? '录音暂时无法播放，可重试。' : '标准普通话'}</span></div>
